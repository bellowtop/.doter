...
this mode runs the hook `bazel-build-mode-hook', as the final or
...
In addition to any hooks its parent mode `bazel-mode' might have run,
this mode runs the hook `bazel-workspace-mode-hook', as the final or
penultimate step during initialization.
...
In addition to any hooks its parent mode `bazel-mode' might have run,
this mode runs the hook `bazel-module-mode-hook', as the final or
penultimate step during initialization.

\\{bazel-module-mode-map}" t)
(add-to-list 'auto-mode-alist (cons (rx 47 (32 (+ nonl) 46) "MODULE.bazel" eos) #'bazel-module-mode))
(autoload 'bazel-repo-mode "bazel"
"Major mode for editing REPO.bazel files.

In addition to any hooks its parent mode `bazel-mode' might have run,
this mode runs the hook `bazel-repo-mode-hook', as the final or
penultimate step during initialization.
...
In addition to any hooks its parent mode `bazel-mode' might have run,
this mode runs the hook `bazel-starlark-mode-hook', as the final or
penultimate step during initialization.
...
In addition to any hooks its parent mode `conf-space-mode' might have
run, this mode runs the hook `bazelrc-mode-hook', as the final or
penultimate step during initialization.
...
this mode runs the hook `bazelignore-mode-hook', as the final or
penultimate step during initialization.
...
abbrev table and syntax table:
  ...
  `bazeliskrc-mode-map', `bazeliskrc-mode-abbrev-table' and
`bazeliskrc-mode-syntax-table'

which more-or-less shadow conf-unix-mode's corresponding tables.

In addition to any hooks its parent mode might have run, this mode
runs the hook `bazeliskrc-mode-hook', as the final or penultimate step
